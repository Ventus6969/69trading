...
## ⏰ v2.7.5 (2025-08-06) - 訂單超時自動取消版

### 🆕 重大新功能
- **🛡️ 智能超時管理系統**
  - 每60秒自動檢查所有未成交訂單，防止資金長期佔用
  - **策略專屬超時設定**: reversal_buy/sell 3小時(180分鐘)，其他策略45分鐘
  - **關聯訂單處理**: 主訂單取消時自動取消相關止盈止損單
  - **智能訂單識別**: 只處理系統訂單(V69_開頭)，跳過手動單和止盈止損單
  - **完整記錄追蹤**: 詳細的取消原因和時間日誌

### 🔧 技術架構升級
- **新增 OrderTimeoutManager 類別**
  - 獨立背景線程，不影響現有交易邏輯
  - 線程安全設計，支援7x24小時運行
  - 完整的異常處理和自動恢復機制
  
- **BinanceClient API 擴充**
  - 新增 `cancel_order_by_client_id()` - 按訂單ID智能取消
  - 新增 `get_order_by_client_id()` - 按訂單ID查詢狀態
  - 自動從訂單ID提取交易對，無需手動指定
...
### 🎯 解決的核心問題
- **資金佔用問題**: reversal_buy訂單6:45開始，9:45自動取消(正好3小時)
- **手動處理負擔**: 無需人工監控和取消超時訂單
- **風險控制增強**: 防止極端市場條件下的長期暴露
- **系統一致性**: 所有策略都有對應的超時保護機制

### 📁 檔案變更清單
- **新增**: `trading/order_timeout_manager.py` (全新超時管理器)
- **修改**: `main.py` (啟動超時管理器)
- **修改**: `api/binance_client.py` (新增取消和查詢方法)
- **修改**: `web/routes.py` (健康檢查增強)
- **修改**: `trading/__init__.py` (模組導入)
- **修改**: `config/settings.py` (版本號和歷史更新)
